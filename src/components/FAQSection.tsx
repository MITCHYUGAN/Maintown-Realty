...
export default function FAQSection() {
  ...
  return (
    <section id="faq" className="py-24 bg-slate-50 relative overflow-hidden border-t border-slate-100">
      {/* Background soft lighting */}
      <div className="absolute top-1/4 left-10 w-72 h-72 bg-indigo-100/50 rounded-full blur-3xl pointer-events-none" />
        ...
        <div className="space-y-4">
          {faqs.map((faq, idx) => {
            ...
            return (
              <div
                key={idx}
                className="rounded-2xl border border-slate-200/60 bg-white overflow-hidden shadow-sm hover:shadow-md hover:border-slate-300/80 transition-all duration-300"
              >
                {/* Header / Trigger */}
                <button
                  id={`faq-trigger-${idx}`}
                ...
                >
                  <span className="font-display text-sm sm:text-base font-bold text-slate-900 tracking-tight pr-4">
                    {faq.question}
                  </span>
                  <div className={`p-1.5 rounded-lg border border-slate-100 transition-all ${isOpen ? 'bg-indigo-50 border-indigo-100 text-indigo-600 rotate-180' : 'bg-slate-50 text-slate-400'}`}>
                ...
                <AnimatePresence initial={false}>
                  {isOpen && (
                    ...
                    >
                      <div className="px-5 pb-5 md:px-6 md:pb-6 text-xs sm:text-sm text-slate-500 leading-relaxed border-t border-slate-50 pt-4">
                        {faq.answer}
                      </div>
                    </motion.div>
                  )}
